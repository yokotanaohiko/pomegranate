...
class RandomAgent(object):
    ...
    def learn(self, obs1, action, reward, obs2, done):
        ...
        if self.q.get(state1) is None:
            self.q[state1] = np.zeros(5) + 0.5
        if self.q.get(state2) is None:
            self.q[state2] = np.zeros(5) + 0.5

        if done:
            self.q[state1][action] += reward * self.importance[action]
            self.q[state1] = normalize(self.q[state1].reshape(1, -1))[0]
        else:
            self.q[state1][action] += reward  * self.importance[action] + 0.7 * np.max(self.q[state2])
            self.q[state1] = normalize(self.q[state1].reshape(1, -1))[0]
...
def train1():
    ...
    env.reset()
    importance = [1,2,3,4,1]
    agent = RandomAgent(env.action_space, importance)

    _obs, _rew, _done = (np.zeros(5).astype(np.int32), None, None)
